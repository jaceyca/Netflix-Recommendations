...
/* DOT PRODUCT FUNCTION. The inline here is CRITICAL because it 
 * preserves cache locality as well as reduces function overhead.
 */
...
/* Calculates root mean square error given U, V, Y, K and the number
 * of points 
 */
...
inline double get_err_w_biases(double** U, double** V, \
    ...
    for (int i = 0; i < NUM_POINTS; i++) {
        int n = int(Y[i][0]) - 1;
        int m = int(Y[i][1]) - 1;
        double rating = Y[i][3] + USER_BIASES[n] + MOVIE_BIASES[m] + average_rating;
        // assert (rating <= 5.0);
        // assert (rating >= 1.0);
...
inline void create_submission(double** U, double** V, \
    ...
    ofstream ofs;
    ofs.open("mfv1_k20_110e_001_001.txt", std::ofstream::out | std::ofstream::trunc);
    for (int i = 0; i < NUM_TEST_POINTS; i++) {
        int n = int(test_set[i][0]) - 1;
        int m = int(test_set[i][1]) - 1;
        double predicted_rating = dot_product(U[n], V[m], K);
        if (predicted_rating < 1.0) {
            predicted_rating = 1.0;
        }
        ...
        ofs << predicted_rating << '\n';
    }
    ofs.close();

}

inline void affirm_size(int test_size) {
    fstream afile; string str; int counter = 0;
    afile.open("mfv1_k20_110e_001_001.txt", ios::out | ios::in); 
    while (std::getline(afile, str)) {
        counter += 1;
    }
    afile.close();
...
inline void change_biases(double** TRAINING_DATA, double* USER_BIASES, double* MOVIE_BIASES, \
    ...
    for (int i = 0; i < NUM_MOVIES; i++) {
        int ri = movie_counting[i];
        double bi = MOVIE_BIASES[i]/(lambda_1 + ri);
        MOVIE_BIASES[i] = bi;
    }
    for (int j = 0; j < NUM_POINTS; j++) {
    ...
    for (int i = 0; i < NUM_USERS; i++) {
        int ru = user_counting[i];
        double bu = USER_BIASES[i]/(lambda_2 + ru);
        USER_BIASES[i] = bu;
    }
    /*
    for (int i = 0; i < NUM_USERS; i++) {
...
int main(int argc, char* argv[]) {
    ...
    if (argc == 5) {
        // We remove global effects. 
        cout << "Initializing Preprocessing: " << endl;
    ...
    if (argc == 5) {
        change_biases(TRAINING_DATA, USER_BIASES, MOVIE_BIASES, user_counting, movie_counting, NUM_POINTS, average_rating);
    }
    ...
    if (argc == 5) { 
        ...
        before_E_in = get_err(U,V, TRAINING_DATA, K, NUM_POINTS);
    }
    cout << "K:  " << K << endl;
    ...
    for (int epoch = 0; epoch < max_epochs; epoch++) {
        ...
        for (int index = 0; index < size; index++) {
            x = TRAINING_DATA[index][0];
            y = TRAINING_DATA[index][1];
            Yij = TRAINING_DATA[index][3];
            double dot_prod = dot_product(U[int(x) - 1], V[int(y) - 1], K); 
            if (argc == 5) {
                dot_prod += average_rating + USER_BIASES[int(x) - 1] + MOVIE_BIASES[int(y) - 1];
            }
            for (int q = 0; q < K; q++) {
                U[int(x) - 1][q] = (U[int(x) - 1][q] * (1 - reg * eta)) + \
                    V[int(y) - 1][q] * (Yij - dot_prod) * eta; 
            }
            dot_prod = dot_product(U[int(x) - 1], V[int(y) - 1], K); 
            for (int q = 0; q < K; q++) {
                V[int(y) - 1][q] = (V[int(y) - 1][q] * (1 - reg * eta)) + \
                    U[int(x) - 1][q] * (Yij - dot_prod) * eta; 
            }
            if (index % marker == 0) {
            // cout << counter << endl;
                cout.flush();
        ...
        if (epoch % 5 == 0) {
            if (argc == 5) {
                ...
                E_in_after = get_err(U,V,TRAINING_DATA, K, NUM_POINTS);
                e_out = get_err(U, V, VALIDATION_DATA, K, NUM_VALIDATION);
            }
    ...
    cout << "Test Set Size: " << qual_data.size() << endl; 
    create_submission(U, V, qual_data, K, qual_data.size());
